...
2. **Live kimlik:** Bot `RunMode::Live` iken Polymarket kimlik bilgileri **yalnızca** SQLite `bot_credentials` satırından okunur (`.env` içindeki `POLY_*` satırları bot süreci tarafından kullanılmaz). `POST /api/bots` gövdesinde `credentials` nesnesi gönder veya UI üzerinden kaydet; aksi halde `MissingCredentials` ile başlatma reddedilir.
