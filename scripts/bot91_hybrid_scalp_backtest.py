#!/usr/bin/env python3
"""Bot 91 — Hybrid: Binance Latency main + Loser Scalp hedge.

Hedge: karşı tarafın bid'i <= max_bid ise FAK BID min_usdc al.
Yanlış yön durumunda hedge tam payoff alır (sigorta).

Test:
  - hedge_usdc: $0, $1, $2, $5, $10
  - hedge_max_bid: 0.20, 0.30, 0.40
"""
...
def sim_session(con, bot_id, sess, btc_lk,
                sig_thr, t_window, main_usdc, hedge_usdc, hedge_max_bid,
    ...
    for ts_ms, ub, ua, db, da in ticks:
        ...
        # Loser scalp hedge: opp_bid çok düşük ise FAK BID al
        if hedge_usdc > 0 and 0.01 <= hedge_bid <= hedge_max_bid:
            hedge_size = math.ceil(hedge_usdc / hedge_bid)
            c_hedge = hedge_size * hedge_bid
            if c_hedge >= 1:  # min order $1 (loser scalp için gevşek)
                cost += c_hedge
                hedge_n += 1
                if opp_dir == w:
    ...
    return dict(cost=cost, pnl=pnl, n=n_t, main_wins=main_wins,
                hedge_n=hedge_n, hedge_wins=hedge_wins)
...
def main():
    ...
    for hedge, mb in sc:
        r = aggregate(con, args.bot_id, sessions, btc_lk,
                      hedge_usdc=hedge, hedge_max_bid=mb, **base_kw)
        results.append((hedge, mb, r))
        marker = " (pure)" if hedge == 0 else ""
        print(f"  ${hedge:>6} {mb:>8.2f} {r['main_wr']:>6.1f}% "
              f"{r['hedge_wr']:>8.1f}% {r['total']:>7} {r['hedge_n']:>7} "
              f"{r['cost']:>10,.0f} {r['net']:>+10,.2f} {r['roi']:>+7.2f}{marker}")

    print(f"\n[NET TOP 10]")
    for i, (h, mb, r) in enumerate(sorted(results, key=lambda x: x[2]["net"], reverse=True)[:10], 1):
        m = " ⭐" if i == 1 else ""
        yearly = r["net"] * (8760 / span_h)
        print(f"  {i:>2}. hedge=${h:<3} max_bid={mb} → "
              f"NET=${r['net']:+10,.2f} ROI={r['roi']:+6.2f}% "
              f"hedge_n={r['hedge_n']:<4} yıllık~${yearly:+,.0f}{m}")
