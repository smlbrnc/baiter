        ...
        <div
          className={cn(
            ...
            current.has_credentials
              ...
              : "bg-background/80 text-muted-foreground shadow-xs"
          )}
          aria-hidden
        >
        ...
        <div className="min-w-0 flex-1 space-y-1 pr-2 sm:pr-3">
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
            ...
            {current.has_credentials ? (
              <Badge
                variant="outline"
                className="border-emerald-500/30 bg-emerald-500/10 px-1.5 py-0 text-[10px] leading-none font-normal text-emerald-600 dark:text-emerald-400"
              >
                <CheckCircle2 className="mr-1 -ml-0.5 size-3" />
                {current.signature_type} · {sigLabel}
              </Badge>
            ) : (
              <Badge
                variant="outline"
                className="border-amber-500/30 bg-amber-500/10 px-1.5 py-0 text-[10px] leading-none font-normal text-amber-600 dark:text-amber-400"
              >
                <XCircle className="mr-1 -ml-0.5 size-3" />
                Henüz kimlik yok
              </Badge>
            )}
          </div>
                ...
                <>
                  ...
                  {current.funder ? (
                    <>
                      {" · funder "}
                      <code className="font-mono">
                        {current.funder.slice(0, 6)}…{current.funder.slice(-4)}
                      </code>
                    </>
                  ) : null}
