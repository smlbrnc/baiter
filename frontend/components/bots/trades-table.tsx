  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-3">
        ...
        >
          <Icon
            className={cn("size-3.5 shrink-0 opacity-80", color)}
            aria-hidden
          />
          {direction} trades
        </CardTitle>
      ...
      <CardContent className="px-0 pb-0">
        <div className="grid grid-cols-[68px_56px_44px_64px_56px_1fr_72px] gap-3 border-y border-border/40 px-4 py-2 text-[10px] font-medium tracking-wider text-muted-foreground uppercase">
          <span>Zaman</span>
          <span>Yön</span>
          <span>Aksiyon</span>
...
function TraderTag({ side }: { side: string | null }) {
  ...
  return (
    <span className="rounded-sm bg-muted/60 px-1.5 py-0.5 text-[10px] font-medium tracking-wide text-muted-foreground uppercase">
