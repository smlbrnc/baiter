  ...
  return (
    <Card>
      ...
      <CardContent className="grid grid-cols-2 gap-4 text-sm sm:grid-cols-4">
        <BestBlock
          title="YES bid"
          v={m.lastBestBidAsk?.yes_best_bid}
          color="green"
          bold
        />
        <BestBlock
          title="YES ask"
          v={m.lastBestBidAsk?.yes_best_ask}
          color="green"
        />
        <BestBlock
          title="NO bid"
        ...
        <Pair
          ...
          secondary={
            m.lastSignal ? `BSI ${m.lastSignal.bsi.toFixed(2)}` : undefined
          }
        />
        <Pair
          title="Last fill"
        ...
        <Pair
          title="Status"
          primary={<Badge variant="outline">{m.lastFill?.status ?? "-"}</Badge>}
        />
      </CardContent>
    </Card>
  );
...
function BestBlock({
  ...
  return (
    <div className="flex flex-col">
      ...
      <span className={klass}>{v == null ? "-" : v.toFixed(4)}</span>
    </div>
  );
}
