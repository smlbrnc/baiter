...
export function BotDetailHeader({
  ...
  return (
    <header className={cn(!marketProgress && "border-b border-border/50")}>
      <div
        className={cn(
      ...
      >
        <div className="flex min-w-0 flex-1 items-start gap-2.5">
          ...
          <div className="min-w-0 flex-1 space-y-1">
            <h1 className="truncate font-heading text-lg leading-snug font-semibold tracking-tight text-foreground">
              {title}
            </h1>
            <div className="flex flex-col gap-1.5 sm:flex-row sm:flex-wrap sm:items-baseline sm:gap-x-2 sm:gap-y-1">
              <div className="min-w-0 font-mono text-[11px] leading-snug break-all text-muted-foreground">
                {subtitle}
              </div>
              {badges ? (
      ...
      {marketProgress ? (
        <div className="space-y-1 px-0.5 pt-2 pb-0.5">
          <div className="relative h-1 w-full overflow-hidden rounded-full bg-muted/50 dark:bg-muted/30">
            <div
